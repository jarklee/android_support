 ...
 *  Author: TrinhQuan. Created on 2017/1/9
...
public final class PermissionManager {
    ...
    public final PermissionManager checkAndRequestPermission(final @Nullable Runnable cancelAction,
                                                             @Nullable final String explainMessage,
                                                             final String... requirePermissions) {
        return checkAndRequestPermission(cancelAction, false, explainMessage, requirePermissions);
    }

    private PermissionManager checkAndRequestPermission(final @Nullable Runnable cancelAction,
        ...
        final int requestId;
        if (cancelAction != null) {
            requestId = cancelAction.hashCode();
        } else {
            requestId = request.hashCode();
        }
        if (permissionRequester.shouldShowExplainMessage() && !StringUtils.empty(explainMessage)) {
            permissionRequester.showExplainMessage(this,
                    explainMessage,
                    requestId,
                    request,
    ...
    private static final class PermissionRequesterImpl implements IPermissionRequester {
        ...
        public void requestPermissions(int requestId, String... permissions) {
            if (activity != null) {
                Utils.requestPermissions(activity, requestId, permissions);
                return;
            }
            if (fragment1 != null) {
                Utils.requestPermissions(fragment1, requestId, permissions);
                return;
            }
            if (fragment2 != null) {
                Utils.requestPermissions(fragment2, requestId, permissions);
            }
        }
        ...
        public boolean hasPermissions(String[] requirePermissions) {
            if (activity != null) {
                return Utils.hasPermissions(activity, requirePermissions);
            }
            if (fragment1 != null) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                    return Utils.hasPermissions(fragment1, requirePermissions);
                }
            }
            if (fragment2 != null) {
                return Utils.hasPermissions(fragment2, requirePermissions);
            }
            return context != null && Utils.hasPermissions(context, requirePermissions);
        }
